...
export default function GeneratePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      {/* ── Tool selector ── */}
      <div className="mb-6 card p-4 sm:p-5 space-y-4 animate-fade-in" style={{ animationDelay: '0.05s' }}>

        {/* Contenu pédagogique */}
        <div>
          <p className="text-[10px] font-semibold text-mg-400 uppercase tracking-widest mb-2.5">
            Contenu pédagogique
          </p>
            ...
            {creationTypes.map(t => (
              <button key={t.value} onClick={() => setType(t.value)}
                className={`p-3 rounded-xl border text-center transition-all duration-200 ${
                  type === t.value
                    ? 'border-accent/40 bg-accent/8 shadow-[0_0_18px_rgba(200,182,255,0.07)]'
                    : 'border-white/5 hover:border-white/12 bg-mg-800/60 hover:bg-mg-800'
                }`}>
                <span className={`text-xl block mb-1 transition-all ${type === t.value ? '' : 'opacity-60'}`}>{t.icon}</span>
                <span className={`text-[11px] font-medium block leading-tight ${type === t.value ? 'text-white' : 'text-mg-400'}`}>{t.label}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {currentTool && (
        <div className="mb-6 flex items-center gap-3.5 p-4 rounded-xl bg-accent/5 border border-accent/12 animate-fade-in">
          <div className="w-9 h-9 rounded-lg bg-accent/10 border border-accent/15 flex items-center justify-center text-lg shrink-0 select-none">
            {currentTool.icon}
          </div>
